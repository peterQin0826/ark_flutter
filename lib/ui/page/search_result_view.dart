...
import 'package:ark/widgets/add_relation_diallog.dart';
...
//  EventBus _eventBus;
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.from == Constant.text_replace_value) {
      ...
      popPage = RouteName.object_item_edit_view;
      _event = Event.search_key;
    } else if (widget.from == Constant.add_relation) {
      popPage = RouteName.object_detail;
      _event = Event.add_relation;
    }
    ...
    return new Scaffold(
      appBar: new AppBar(
        ...
        actions: <Widget>[
          IconButton(
            ...
            onPressed: () {
              print('被选择的主键：${_data.basicData.objKey}');
              if (_data == null) {
                Toast.show('请先选择');
                return;
              }
              if (widget.from == Constant.add_relation) {
                showDialog(
                    context: context,
                    builder: (context) {
                      return AddRelationDialog(
                        onConfirm: (relNum, score, createMode, data) {
                          _model
                              .relationAdd(widget.objKey,
                                  _data.basicData.objKey, createMode,
                                  rel_number: relNum, score: score, data: data)
                              .then((value) {
                            if (value) {
                              Toast.show('创建关联成功');
      ...
      body: ProviderWidget<SearchResultModel>(
